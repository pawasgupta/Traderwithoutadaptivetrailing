...
class Trader:
    def __init__(self):         #constructor to initialise the members
        ...
        self.m_str_RinaFileName="RinaCSVOutput" # Rina File name
    ...
    def m_CreateOHLC(self,l_TickMatrix,l_NumberOfTicks):
        ...
        barOpen=round(float(l_TickMatrix[0][2]),2) # Open in 3rd column of TickMatrix
        barClose=round(float(l_TickMatrix[l_NumberOfTicks-1][5]),2)


        RoundedHigh = [ round((l_TickMatrix[temp][3]), 2) for temp in range(0,l_NumberOfTicks) ] # High in 4th column of TickMatrix
        barHigh = max(RoundedHigh)

        RoundedLow = [ round((l_TickMatrix[temp][4]), 2) for temp in range(0,l_NumberOfTicks) ] #Close in 5th column of TickMatrix
    ...
    def m_TradingAlgorithm(self):
        ...
        self.m_f_Profit[self.m_i_t-1]=self.m_i_Position[self.m_i_t-2]*(l_f_ClosePrice[self.m_i_t-1]-l_f_ClosePrice[self.m_i_t-2])-0.035*abs(self.m_i_Position[self.m_i_t-1]-self.m_i_Position[self.m_i_t-2])   #instantaneous profit
